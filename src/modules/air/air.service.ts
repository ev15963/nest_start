...
//  }
  ...
  async upsert() {
   ...
   // 'https://openapi.gg.go.kr/Sidoatmospolutnmesure?TYPE=jso/',
    ...
    const {
      ...
      = await value.data.Sidoatmospolutnmesure[1].row[0];

    const query_result = await this.dataSource
      .createQueryBuilder()
      .insert()
      .into(TBL_AIR)
      .values({
     ...
     .onConflict(`("mesrnw_nm") DO UPDATE SET 
